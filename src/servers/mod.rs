...
fn _server_list_callback<F>(mut callback: F)
where
  F: FnMut(&mut MutexGuard<Vec<structs::Server>>),
{
  match unsafe { SERVER_LIST.lock() } {
    ...
    Err(e) => log!(
      "error",
      format!(
        "something went wrong while trying to lock `SERVER_LIST`\n{e}"
      )
    ),
  }
}
...
pub fn get_list() -> Vec<structs::Server> {
  ...
  _server_list_callback(|list| result = list.clone());
  result
}
